...
x.knots.list <- list(knots1=seq(0,1,1/5),knots2=seq(0,1,1/5))
...
pred_error_cv <- function(X_list,Y,nfold=10){
   ...
   colnames(pse_M_matrix) <- c("env_dir","full_dir","u_env_dir","env_KL","full_KL","pcr","pls","u_env_KL","u_pcr","u_pls")
   # pse_M_matrix <- matrix(unlist(pse),byrow = TRUE,ncol = 10)
...
pse <- pred_error_cv(X_list,Y,nfold = 3)
colMeans(pse$pse)
